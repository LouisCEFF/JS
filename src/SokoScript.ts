...
const dirselect: any = document.getElementById('dirs');
dirselect.addEventListener("change", async () => {
    loadLvlsForDir();
    init();
});
const lvlselect: any = document.getElementById('lvls');
...
type SokoCrateMoves = SokoCM[];

let rowNum: number = 1;
let colNum: number = 0;
let sokobanSpace: SokoGrid;
let sokobanCrates: SokoCrates;
let playerMoves: SokoPlayerMoves;
let crateMoves: SokoCrateMoves;
let onGoals: number = 0;
let direction: CharSet;
const sokoban = document.getElementById("SOKOBAN") as HTMLCanvasElement;
/** @type {CanvasRenderingContext2D} */
const ctx = sokoban.getContext("2d") as CanvasRenderingContext2D;
init();
...
interface IPLayer {
    ...
    y: number;
    onGoal: boolean;
    move: { (dx: number, dy: number, grid: SokoGrid): any };
    back: { (): any };
    facing: { (direction: string): any };
}
...
const player: IPLayer = {
    ...
    y: 0,
    onGoal: false,
    move: function (dx: number, dy: number, grid: SokoGrid) {
    ...
    back: function () {
        ...
        let newPlayerPos: SokoPM | undefined = playerMoves[playerMoves.length - 2];
        const index: number = playerMoves.findIndex((pos) => pos.x == newPlayerPos!.x && pos.y == newPlayerPos!.y);
        if (index < 0) {
        ...
        let newCratePos: SokoPM | undefined = playerMoves[playerMoves.length - 1];
        if (newCratePos!.crateMoved == true) {
            const crateIndex: number = crateMoves.findIndex((c) => c.x == newCratePos!.x && c.y == newCratePos!.y);
            if (crateIndex == -1) {
                return;
            }
            const crate: SokoCM | undefined = crateMoves.find((c) => c.x == newCratePos!.x && c.y == newCratePos!.y);
            const thisCratePos: any[] = [];
            ...
            const crates: SokoC | undefined = sokobanCrates.find((c) => c.x == lastCratePos.x && c.y == lastCratePos.y);
            if (sokobanSpace[crates!.y]![crates!.x]!.char == ".") {
                const crateI: number = sokobanCrates.findIndex((c) => c.x == crates!.x && c.y == crates!.y)
...
function fillSelLvls(levels: any) {
    lvlselect.innerHTML = "";
    for (let l of levels) {
...
function initLevel(data: string[]) {
    testGame = data;
}
function refresh() {
    ...
    for (let y = 0; y < colNum; y++) {
        for (let x = 0; x < rowNum; x++) {
            ...
            let char: CharSet | undefined = sokobanSpace[y]![x]?.char;
            if (char == undefined)
                return;
            ...
            let image = images(charToAction[char]) as CanvasImageSource;
            if (char != " ") {
                ctx.drawImage(images(charToAction[' ']) as CanvasImageSource, x * cellSize, y * cellSize);
            ...
            if (player.onGoal == true && player.x == x && player.y == y) {
                ctx.drawImage(images(charToAction["+"]) as CanvasImageSource, player.x * cellSize + 5, player.y * cellSize + 5, 50, 50);
            } else if (player.onGoal == false && player.x == x && player.y == y) {
                ctx.drawImage(images(charToAction[direction]) as CanvasImageSource, player.x * cellSize + 5, player.y * cellSize + 5, 50, 50);
            }
        }
    }
    for (let c of sokobanCrates) {
        if (sokobanSpace[c!.y]![c!.x]!.char == "." && c.onGoal == true) {
            ctx.drawImage(images(charToAction["*"]) as CanvasImageSource, c.x * cellSize + 5, c.y * cellSize + 5, 50, 50);
        }
...
function gameOver() {
    let overlay: HTMLElement | null = document.getElementById("overlay");
    if (playable == true) {
...
function moveCrate(dy: number, dx: number, x: number, y: number) {
    for (let c of sokobanCrates) {
        if (c.x == x && c.y == y) {
